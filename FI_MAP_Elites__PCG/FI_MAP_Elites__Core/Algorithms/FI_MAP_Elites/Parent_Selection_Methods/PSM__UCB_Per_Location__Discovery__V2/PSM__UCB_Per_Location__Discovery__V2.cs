...
{
    public class PSM__UCB_Per_Location__Discovery__V2<T> : Parent_Selection_Method<T>,
        ...
        where T : Data_Structure
    {
        ...
        public PSM__UCB_Per_Location__Discovery__V2(
            double c_value,
            ...
            )
        {
            ...
            for (int c = 0; c < tessellation.num_cells; c++)
            {
                ...
                feasible_state__rewards__archive[c] = new double[queue_length];
                ...
                infeasible_state__rewards__archive[c] = new double[queue_length];
                infeasible_state__ucb_values__archive[c] = double.NegativeInfinity;

                for (int i = 0; i < queue_length; i++)
                {
                    feasible_state__rewards__archive[c][i] = 0.0;
                    infeasible_state__rewards__archive[c][i] = 0.0;
                }
            }
        }

        private PSM__UCB_Per_Location__Discovery__V2(PSM__UCB_Per_Location__Discovery__V2<T> psm_to_copy)
        {
            ...
            this.queue_length = psm_to_copy.queue_length;
        ...
        {
            return new PSM__UCB_Per_Location__Discovery__V2<T>(this);
        }
        ...
        {
            queue_phase++;
            queue_phase = queue_phase % queue_length;
            ...
            if (score_increase)
            {
                if (event_args.parent_location.state == FIME__State_Type.Feasible)
                {
                    for (int c = 0; c < tessellation.num_cells; c++)
                    {
                        if (c == parent_cell)
                        {
                            feasible_state__rewards__archive[c][queue_phase] = 1.0;
                        }
                        else
                        {
                            feasible_state__rewards__archive[c][queue_phase] = 0.0;
                        }
                    }
                }
                else
                {
                    for (int c = 0; c < tessellation.num_cells; c++)
                    {
                        if (c == parent_cell)
                        {
                            infeasible_state__rewards__archive[c][queue_phase] = 1.0;
                        }
                        else
                        {
                            infeasible_state__rewards__archive[c][queue_phase] = 0.0;
                        }
                    }
                }
            }
